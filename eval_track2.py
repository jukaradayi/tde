...
""")
    ...
    fragments_cross_file  = path.join(resource_dir, '{}.intervals.cross'.format(args.corpus))
    fragments_within_file = path.join(resource_dir, '{}.intervals.within'.format(args.corpus))
    ...
    split_file            = path.join(resource_dir, '{}.split'.format(args.corpus))
    ...
    with verb_print('  loading word corpus file',
                             verbose, True, True, True):
    wrd_corpus = _load_corpus_txt(wrd_corpus_file)

    with verb_print('  loading phone corpus file',
                             verbose, True, True, True):
    phn_corpus = _load_corpus_txt(phn_corpus_file)
    
    # load across and withing folds
    with verb_print('  loading folds cross',
                             verbose, True, True, True):
        fragments_cross = load_split(fragments_cross_file,
                                     multiple=False)
    with verb_print('  loading folds within',
                             verbose, True, True, True):
        fragments_within = load_split(fragments_within_file,
                                      multiple=False)
    ...
    with verb_print('  loading gold classes',
                             verbose, True, True, True):
        gold_clsdict = _load_classes(gold_clsfile, phn_corpus)
